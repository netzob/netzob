...
//        alignTwoMessages(&tmpMessage, doInternalSlick, &groups->groups[i].messages[m], &groups->groups[p].messages[n], debugMode);
        similarityScore += NeedlemanScore(&groups->groups[i].messages[m], &groups->groups[p].messages[n], debugMode);
...
float NeedlemanScore(t_message * message1, t_message * message2, Bool debugMode){
  ...
  //+------------------------------------------------------------------------+
  matrix = (unsigned int**) malloc( sizeof(unsigned int*) * (message1->len + 1) );
  for (i = 0; i < (message1->len + 1); i++) {
    matrix[i] = (unsigned int*) calloc( (message2->len + 1), sizeof(unsigned int) );
