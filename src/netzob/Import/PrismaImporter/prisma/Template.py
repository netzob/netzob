...
class Template(object):
    ...
    def __str__(self):
        return 'ID {!s} {!s} {!s} {!s}'.format(self.ID, self.state, self.count,
                                               self.fields)  # str(self.ID) +' '+ self.state
    ...
    def isServer(self, role=False):
        if role:
            return 'UAS' not in self.state.getCurState()
        return 'UAS' in self.state.getCurState()
