...
class SequenceTableView(object):
    ...
    def __init__(self, controller):
        ...
        self.builder.add_from_file(os.path.join(
            ...
            "ui", "vocabulary",
            "sequenceTable.glade"))
        self._getObjects(self.builder, ["sequenceTableBox",
                                        "fieldNameLabel",
                                        "sequenceTableScrolledWindow"])
        self.builder.connect_signals(self.controller)
        self.displayedObject = None
        # Make an empty treeview
        self.sequenceTableTreeView = self.__makeMessageTreeView()
        self.sequenceTableScrolledWindow.add(self.sequenceTableTreeView)
        self.sequenceTableTreeView.show()
    ...
    def __makeMessageTreeView(self):
        ...
        sequenceTableTreeView = Gtk.TreeView()
        sequenceTableTreeView.connect("enter-notify-event", self.controller.sequenceTableTreeView_enter_notify_event_cb)
        sequenceTableTreeView.connect("leave-notify-event", self.controller.sequenceTableTreeView_leave_notify_event_cb)
        sequenceTableTreeView.connect("button-press-event", self.controller.sequenceTableTreeView_button_press_event_cb)
        sequenceTableTreeView.get_selection().set_mode(Gtk.SelectionMode.MULTIPLE)
        sequenceTableTreeView.get_selection().connect("changed", self.controller.sequenceTableTreeView_changed_event_cb)
        sequenceTableTreeView.set_rules_hint(True)
        sequenceTableTreeView.set_grid_lines(Gtk.TreeViewGridLines.BOTH)
        # Configures it as a Drag Source
        sequenceTableTreeView.enable_model_drag_source(Gdk.ModifierType.BUTTON1_MASK, [], Gdk.DragAction.MOVE)
        sequenceTableTreeView.connect("drag-data-get", self.__drag_data_get_event)
        sequenceTableTreeView.drag_source_add_text_targets()

        # Create columns
        if self.displayedObject is None or len(self.displayedObject.getExtendedFields()) < 1:
            return sequenceTableTreeView
        ...
        for colIdx in range(startOfColumns, numOfColumns):
            ...
            sequenceTableTreeView.append_column(tvc)
            but = tvc.get_button()
        ...
        columns = sequenceTableTreeView.get_columns()
    ...
    def updateSequenceTableTreeView(self):
    ...
    def updateSequenceTableListStore(self):
        ...
        # Split every message
        logging.debug("Align {0} messages with regex {1}".format(len(messages), self.displayedObject.getRegex()))
        ...
        self.sequenceTableListStore = Gtk.ListStore(*listStoreTypes)
        # Fill listStore with split messages
        for splitMessage in splitMessagesMatrix:
            self.sequenceTableListStore.append(splitMessage)
        self.sequenceTableTreeView.set_model(self.sequenceTableListStore)
    ...
    def sortMessagesByField(self, field, sortType):
        ...
        self.sequenceTableListStore.set_sort_column_id(sortIndex, sortTypeMap[sortType])
    ...
    def setSelected(self, selected):
        ...
        if selected:
            ...
            selection = self.sequenceTableTreeView.get_selection()
    ...
    def update(self):
        ...
        self.updateSequenceTableTreeView()

    def getPanel(self):
        return self.sequenceTableBox

    def destroy(self):
        self.sequenceTableBox.destroy()


class TreeViewHeaderWidget(Gtk.VBox):
    """Header Widget for columns of TreeViews displaying sequence messages.
    This header is designed to display properties of a sequence field, like :
    - Name
    - Corresponding Regex
    - Display format
    ...
    def __init__(self, field, treeViewColumn, sequenceTableView):
        Gtk.VBox.__init__(self)
        self.field = field
        self.treeViewColumn = treeViewColumn
        self.sequenceTableView = sequenceTableView
        self.__setupUI()
...
#            self.__setState(self.STATE_UNFOCUSED)
    ...
    def formatEventBox_button_press_event_cb(self, *args):
        ...
        self.sequenceTableView.updateSequenceTableListStore()

    def sortEventBox_button_press_event_cb(self, *args):
        if self.sortStatus == self.SORT_NONE:
            self.sequenceTableView.sortMessagesByField(self.field, self.SORT_DESCENDING)
            self.setSortIndicator(self.SORT_DESCENDING)
        elif self.sortStatus == self.SORT_DESCENDING:
            self.sequenceTableView.sortMessagesByField(self.field, self.SORT_ASCENDING)
            self.setSortIndicator(self.SORT_ASCENDING)
        elif self.sortStatus == self.SORT_ASCENDING:
            self.sequenceTableView.sortMessagesByField(None, self.SORT_ASCENDING)
            self.setSortIndicator(self.SORT_NONE)
    ...
    def titleEventBox_button_press_event_cb(self, *args):
        self.sequenceTableView.controller.vocabularyController.setSelectedMessageTable(self.sequenceTableView)
        self.setSelected(not self.selected)
        self.sequenceTableView.updateBackgroundColor(self)

        # Emit Signals to update toolbar status
        nbSelectedFields = len(self.sequenceTableView.treeViewHeaderGroup.getSelectedFields())
        signalsManager = self.sequenceTableView.controller.getSignalsManager()
