...
class SymbolTableView(object):
    ...
    def __init__(self, controller):
        ...
        self.builder.add_from_file(os.path.join(
            ...
            "ui", "vocabulary",
            "symbolTable.glade"))
        self._getObjects(self.builder, ["symbolTableBox",
                                        "fieldNameLabel",
                                        "symbolTableScrolledWindow"])
        self.builder.connect_signals(self.controller)
        self.displayedObject = None
        # Make an empty treeview
        self.symbolTableTreeView = self.__makeMessageTreeView()
        self.symbolTableScrolledWindow.add(self.symbolTableTreeView)
        self.symbolTableTreeView.show()
    ...
    def __makeMessageTreeView(self):
        ...
        symbolTableTreeView = Gtk.TreeView()
        symbolTableTreeView.connect("enter-notify-event", self.controller.symbolTableTreeView_enter_notify_event_cb)
        symbolTableTreeView.connect("leave-notify-event", self.controller.symbolTableTreeView_leave_notify_event_cb)
        symbolTableTreeView.connect("button-press-event", self.controller.symbolTableTreeView_button_press_event_cb)
        symbolTableTreeView.get_selection().set_mode(Gtk.SelectionMode.MULTIPLE)
        symbolTableTreeView.get_selection().connect("changed", self.controller.symbolTableTreeView_changed_event_cb)
        symbolTableTreeView.set_rules_hint(True)
        symbolTableTreeView.set_grid_lines(Gtk.TreeViewGridLines.BOTH)
        # Configures it as a Drag Source
        symbolTableTreeView.enable_model_drag_source(Gdk.ModifierType.BUTTON1_MASK, [], Gdk.DragAction.MOVE)
        symbolTableTreeView.connect("drag-data-get", self.__drag_data_get_event)
        symbolTableTreeView.drag_source_add_text_targets()
        ...
        for colIdx in range(startOfColumns, numOfColumns):
            ...
            symbolTableTreeView.append_column(tvc)
            but = tvc.get_button()
        ...
        columns = symbolTableTreeView.get_columns()
    ...
    def refreshProperties(self):
        ...
        self.symbolTableTreeView.queue_draw()
    ...
    def updateSymbolTableTreeView(self):
        ...
        if self.symbolTableTreeView is not None:
            self.symbolTableScrolledWindow.remove(self.symbolTableTreeView)
        if self.displayedObject is None:
            return
        ## Create a new treeview
        self.symbolTableTreeView = self.__makeMessageTreeView()
        ## Create and fill store for the create tree view
        self.updateSymbolTableListStore()
        ## Display newly created treeview
        self.symbolTableScrolledWindow.add(self.symbolTableTreeView)
        self.symbolTableTreeView.show()
        logging.debug("End to update the message table")

    def updateSymbolTableListStore(self):
    ...
    def setSelected(self, selected):
        ...
        if selected:
            ...
            selection = self.symbolTableTreeView.get_selection()
    ...
    def update(self):
        ...
        self.updateSymbolTableTreeView()

    def getPanel(self):
        return self.symbolTableBox

    def destroy(self):
        self.symbolTableBox.destroy()
...
class TreeViewHeaderWidget(Gtk.VBox):
    ...
    def __init__(self, field, treeViewColumn, symbolTableView):
        Gtk.VBox.__init__(self)
        self.field = field
        self.treeViewColumn = treeViewColumn
        self.symbolTableView = symbolTableView
        self.__setupUI()
...
#            self.__setState(self.STATE_UNFOCUSED)
    ...
    def formatEventBox_button_press_event_cb(self, *args):
        ...
        self.symbolTableView.updateSymbolTableListStore()

    def sortEventBox_button_press_event_cb(self, *args):
        if self.sortStatus == self.SORT_NONE:
            self.symbolTableView.sortMessagesByField(self.field, self.SORT_DESCENDING)
            self.setSortIndicator(self.SORT_DESCENDING)
        elif self.sortStatus == self.SORT_DESCENDING:
            self.symbolTableView.sortMessagesByField(self.field, self.SORT_ASCENDING)
            self.setSortIndicator(self.SORT_ASCENDING)
        elif self.sortStatus == self.SORT_ASCENDING:
            self.symbolTableView.sortMessagesByField(None, self.SORT_ASCENDING)
            self.setSortIndicator(self.SORT_NONE)
    ...
    def titleEventBox_button_press_event_cb(self, *args):
        self.symbolTableView.controller.vocabularyController.setSelectedMessageTable(self.symbolTableView)
        self.setSelected(not self.selected)
        self.symbolTableView.updateBackgroundColor(self)

        # Emit Signals to update toolbar status
        nbSelectedFields = len(self.symbolTableView.treeViewHeaderGroup.getSelectedFields())
        signalsManager = self.symbolTableView.controller.getSignalsManager()
