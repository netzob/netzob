...
class Symbol(AbstractField):
    ...
    **Usage of Symbol for protocol modeling**
    ...
    def count(self, preset: Preset = None) -> int:
        ...
        >>> preset = Preset()
        >>> preset[f2] = 42
        >>> symbol.count(preset=preset)  # Here, the following computation is done: 951*1*256 (as the f2 field value is set to 42, f2 can now produce only 1 possible value)
        243456
        >>>
        >>> # Specify a preset configuration for field 'f3' by activating fuzzing
        >>> preset.fuzz(f3, generator='determinist')
        >>>
        >>> symbol.count(preset=preset)  # Here, the following computation is done: 951*1*29 (29 corresponds to the number of possible values generated by the determinist generator)
        27579
