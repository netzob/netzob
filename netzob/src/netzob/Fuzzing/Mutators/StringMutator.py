...
def _test_string_length():
    ...
    >>> len_list = []
    >>> mutator = StringMutator(fieldString.domain, interval=(10,600), seed=10, lengthBitSize=UnitSize.SIZE_8)
    >>> len_list.append(len(mutator.generate()))
    >>> mutator = StringMutator(fieldString.domain, interval=(10,600), seed=10, lengthBitSize=UnitSize.SIZE_16)
    >>> len_list.append(len(mutator.generate()))
    >>> mutator = StringMutator(fieldString.domain, interval=(10,600), seed=10, lengthBitSize=UnitSize.SIZE_4)
    >>> len_list.append(len(mutator.generate()))
    >>> len_list
    [255, 65535, 15]
    >>> len_is_good = True
...
def _test_string_value():
    ...
    >>> mutator = StringMutator(fieldString.domain, interval=(10,600), seed=10, lengthBitSize=UnitSize.SIZE_8, endchar=eos_symbol)
    >>> naughty_string = StringPaddedGenerator.DEFAULT_NAUGHTY_STRINGS
    
    >>> has_naughty_str = False
    >>> has_several_eos_symbol = False
    >>> for _ in range(20):
    ...     str = mutator.generate()
    ...     if str in naughty_string:
